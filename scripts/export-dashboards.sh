...
RAW="$EXPORT_DIR/.raw_export.zip"
CONTAINER_TMP="/tmp/gigaid_dashboards.zip"
# какие дашборды оставить в чистом экспорте (regex по заголовку); остальное (демо
# Superset и т.п.) отбрасывается вместе с неиспользуемыми чартами/датасетами
KEEP_PATTERN="${KEEP_PATTERN:-GigaID}"
...
echo "==> Копирую полный экспорт из контейнера"
CID="$(dc ps -q superset)"
docker cp "$CID:$CONTAINER_TMP" "$RAW"
dc exec -T superset rm -f "$CONTAINER_TMP" >/dev/null 2>&1 || true

echo "==> Оставляю только дашборд(ы) по шаблону «$KEEP_PATTERN» + их зависимости"
KEEP_PATTERN="$KEEP_PATTERN" python3 "$SCRIPT_DIR/_prune_export.py" "$RAW" "$DEST"
rm -f "$RAW"
